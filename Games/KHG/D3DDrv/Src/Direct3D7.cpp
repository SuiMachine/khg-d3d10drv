...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
	...
	static void StaticConstructor( UClass* Class )
	{
		...
		new(Class,TEXT("UseMipmapping"),       RF_Public)UBoolProperty ( CPP_PROPERTY(UseMipmapping       ), TEXT("Options"), CPF_Config );
		...
		new(Class,TEXT("UsePalettes"),         RF_Public)UBoolProperty ( CPP_PROPERTY(UsePalettes         ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseGammaCorrection"),  RF_Public)UBoolProperty ( CPP_PROPERTY(UseGammaCorrection  ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("Use3dfx"),             RF_Public)UBoolProperty ( CPP_PROPERTY(Use3dfx             ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseTripleBuffering"),  RF_Public)UBoolProperty ( CPP_PROPERTY(UseTripleBuffering  ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseVSync"),            RF_Public)UBoolProperty ( CPP_PROPERTY(UseVSync            ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UsePrecache"),         RF_Public)UBoolProperty ( CPP_PROPERTY(UsePrecache         ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseVideoMemoryVB"),    RF_Public)UBoolProperty ( CPP_PROPERTY(UseVideoMemoryVB    ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseAGPTextures"),      RF_Public)UBoolProperty ( CPP_PROPERTY(UseAGPTextures      ), TEXT("Options"), CPF_Config );		
		new(Class,TEXT("UseVertexFog"),        RF_Public)UBoolProperty ( CPP_PROPERTY(UseVertexFog        ), TEXT("Options"), CPF_Config );		
		new(Class,TEXT("Use32BitZBuffer"),     RF_Public)UBoolProperty ( CPP_PROPERTY(Use32BitZBuffer     ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("UseDetailTextures"),   RF_Public)UBoolProperty ( CPP_PROPERTY(UseDetailTextures   ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("dwVendorId"),          RF_Public)UIntProperty  ( CPP_PROPERTY(dwVendorId          ), TEXT("Options"), CPF_Config );
		new(Class,TEXT("dwDeviceId"),          RF_Public)UIntProperty  ( CPP_PROPERTY(dwDeviceId          ), TEXT("Options"), CPF_Config );
	
#if 0 /* Moved to SetRes() */
		UseVertexSpecular		= 1; 
		UseAlphaPalettes		= 1;
		SpanBased				= 0;
		SupportsFogMaps			= 1;
	...
	UBOOL Init( UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		// Driver flags.
		SpanBased			= 0;
		FullscreenOnly		= 1;//!!drv dependent
		SupportsFogMaps		= 0;
		SupportsDistanceFog	= 0;
	...
	void DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet )
	{
		...
		if((Surface.DetailTexture && Surface.FogMap) || (!UseDetailTextures))
	...
	void SetTexture( DWORD dwStage, FTextureInfo& Info, DWORD PolyFlags, UBOOL Precache )
	{
		...
		if( !ti )
		{
			...
			if( Info.Format==TEXF_DXT1 )
				...
				appErrorf(TEXT("Unknown Unreal Texture Format"));
		...
		if( (Info.TextureFlags & TF_RealtimeChanged) || (Info.Format==TEXF_RGBA7 && GET_COLOR_DWORD(*Info.MaxColor)==0xFFFFFFFF) )
		{
			// Get ready for blt.
#if 0
	...
	Retry:
		...
		for( TArray<FDeviceInfo>::TIterator It(DirectDraws); It; ++It )
			if( appStrfind( *It->Description, TEXT("Primary") ) )
				Best = *It;
		if( LocalUse3dfx )
			for( TArray<FDeviceInfo>::TIterator It(DirectDraws); It; ++It )
				if( appStrfind( *It->Description, TEXT("Primary") ) )
		...
		if( Viewport==NULL )
		{
			...
			{
				TCHAR Text[4096];//!!
				appSprintf( Text, TEXT("%i"),DescFlags/*|RDDESCF_Certified*/ );
				SetConfigString(TEXT("GlideDrv.GlideRenderDevice"),TEXT("DescFlags"),Text);
			}
		...
		INT bLowDetail;
		guard(bLowDetail);
		bLowDetail = Viewport->GetOuterUClient()->LowDetailTextures;
		unguard;
		DWORD OptSize = 32;
		...
		DWORD USize = 0xDEADBEEF;
		DWORD VSize = 0xDEADBEEF;
		...
		for( Count=0; Count<MaxCount; Count++ )
		{
			...
			for( FPixFormat* Fmt=FirstPixelFormat; Fmt; Fmt=Fmt->Next )
			{
				check(Fmt);
				guard(USizeLoop);
				for( USize=DeviceDesc7.dwMinTextureWidth; USize<=Fmt->MaxWidth; USize*=2 )
				{
					guard(VSizeLoop);
					for( VSize=DeviceDesc7.dwMinTextureHeight; VSize<=Fmt->MaxHeight; VSize*=2 )
					{
						...
						FTexPool* Pool;
						guard(FindPool);
						Pool = Fmt->PoolList.FindRef(SizeKey);
						guard(Pool);
						if( Pool )
						{
		...
		unguardf(( TEXT("(Count=%i,USize=%i,VSize=%i)"), Count, USize, VSize ));
	...
	FString D3DError( HRESULT h )
	{
		...
		switch( h )
		{
			...
			default:
				TCHAR Text[4096];//!!
				appSprintf(Text,TEXT("%08X"),(INT)h);
				return FString( Text );
		}
		#undef D3DERR
	}
